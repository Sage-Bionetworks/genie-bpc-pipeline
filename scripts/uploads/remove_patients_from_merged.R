# Description: Remove patients from merged and uncoded REDCap data exports.  
...
if (verbose) {
  print(glue("{now()}: removing {length(pt_rm)} patients from {file_local} (synid_file_input).."))
}

raw <- get_synapse_entity_data_in_csv(synid_file_input, na.strings = c(""))
mod <- raw %>% 
  filter(!is.element(record_id, pt_rm))
write.csv(mod, file = file_local, row.names = F, na = "")
...
if (!is.na(synid_folder_output)) {
  synid_file_dest <- save_to_synapse(path = file_local, 
                  ...
                  prov_name = "filtered merged file", 
                  prov_desc = "remove redacted patients from files as temporary fix for public release", 
                  prov_used = c(synid_file_input, synid_table_rm), 
                  prov_exec = "https://github.com/Sage-Bionetworks/genie-bpc-pipeline/blob/develop/scripts/release/remove_patients_from_merged.R")
  ...
  if (verbose) {
    print(glue("{now()}: '{file_local}' written locally to {getwd()}"))
  }
}
