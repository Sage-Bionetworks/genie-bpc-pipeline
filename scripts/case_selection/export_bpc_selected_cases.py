...
def main():
    ...
    # The uploaded file expects a csv, but removePandasDfFloat exports a tsv
    ...
    # HACK: if the results don't match in md5 from the
    # original upload, it is because R writes tsv with "quotes" around it
    # while pandas doesn't
    # patient_output.to_csv(output_file_name, index=False, na_rep="")

    if args.save_synapse:
        # create an Activity
        act = Activity(
