...
params:
  ...
  cohort: x
  site: x
  main_patient: "`r paste(x, 'Main Genie clinical patient file version:', synGet(x, followLink = TRUE)$versionComment, sep = ' ')`"
  main_sample: "`r paste(x, 'Main Genie clinical sample file version:', synGet(x, followLink = TRUE)$versionComment, sep = ' ')`"
---
...
site <- params$site
main_patient <- params$main_patient
main_sample <- params$main_sample
...
**Main GENIE Clinical Patient File**: `r main_patient`

**Main GENIE Clinical Sample File**: `r main_sample`
...
```{r eligibility}
n_main_genie <- read.csv(synGet(main_patient, followLink = TRUE)$path, sep = "\t", header = TRUE, skip=4) %>% filter(CENTER == site) %>%  nrow()
