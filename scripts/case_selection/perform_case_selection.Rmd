...
```{r eligibility}
main_clinical <- get_main_genie_clinical_ids(release)

n_main_genie <- read.csv(synGet(main_clinical$main_patient, followLink = TRUE)$path, sep = "\t", header = TRUE, skip=4) %>% filter(CENTER == site) %>%  nrow()
