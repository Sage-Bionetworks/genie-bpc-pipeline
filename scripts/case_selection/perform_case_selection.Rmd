...
```{r eligibility}
main_clinical <- get_main_genie_clinical_id(release)

n_main_genie <- read.csv(synGet(main_clinical, followLink = TRUE)$path, sep = "\t", header = TRUE) %>% filter(CENTER == site) %>%  nrow()
...
print(glue("Target number of SDV cases (including pressure): {get_sdv(config, phase, cohort, site) + get_pressure(config, phase, cohort, site)}"))
