...
parser.add_argument("-c", "--comment", type=str, default="update to case selection criteria",
...
def get_phases(config):
    phases = [name for name in config["phase"].keys() if not name.endswith("_additional")]
...
# main ----------------------------
# storage
labels = ["phase", "cohort", "site", "min_seq_date", "max_seq_date", "oncotree", "target"]
...
if save_synapse:
  if verbose:
    print(f"Update case selection criteria table ({config['synapse']['selection_criteria']['id']})...")
