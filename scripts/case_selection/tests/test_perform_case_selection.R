...
test_that("create_selection_matrix throws an error when not enough eligible patients", {
  ...
  expect_error(create_selection_matrix(eligible_cohort, n_prod, n_pressure, n_sdv, n_irr), 
...
test_that("create_selection_matrix correctly categorizes cases", {
  ...
  # call the function
  categorized_cohort <- create_selection_matrix(eligible_cohort, n_prod, n_pressure, n_sdv, n_irr)
  ...
  expect_equal(sum(categorized_cohort$sdv == "sdv"), n_sdv)
  expect_equal(sum(categorized_cohort$irr == "irr"), n_irr)
  expect_equal(sum(categorized_cohort$category == "production"), n_prod)
  expect_equal(sum(categorized_cohort$category == "extra"), nrow(eligible_cohort) - n_prod)
})