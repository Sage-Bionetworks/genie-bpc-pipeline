...
parser.add_argument("-s", "--save_synapse", action='store_true', default=False, 
                    help="Save updated counts on Synapse")
parser.add_argument("-c", "--comment", type=str,
                    help="Comment for new table snapshot version")
parser.add_argument("-a", "--synapse_auth", type=str, default=None,
...
def create_synapse_table_snapshot(table_id, comment):
    snapshot = syn.restPOST(f"/entity/{table_id}/table/transaction/commit", 
...
# main ----------------------------
labels = ["cohort", "site", "phase", "current_cases", "target_cases", "adjusted_cases", "pressure", "sdv", "irr"]
...
for phase in config['phase'].keys():
    for cohort in config['phase'][phase]['cohort'].keys():
        for site in config['phase'][phase]['cohort'][cohort].get('site',  {}).keys():
            ...
            final = final.append(pd.Series([cohort, site, phase, n_current, n_target, n_adjust, n_pressure, n_sdv, n_irr], index=labels), ignore_index=True)
...
# close out ----------------------------
if save_synapse:
    print(f"Table saved to Synapse as 'Case Selection Counts' ({config['synapse']['case_selection']['id']}), version {n_version}")
else:
