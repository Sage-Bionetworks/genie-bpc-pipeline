...
def get_pressure(config: dict, phase: str, cohort: str, site: str):
    ...
    n_pressure_specific = config['phase'][phase]['cohort'][cohort]['site'][site].get('pressure', None)
    n_pressure = n_pressure_specific if n_pressure_specific is not None else n_pressure_default
    return n_pressure
...
def get_synapse_folder_children(synapse_id: str, include_types: List[str] = ["folder", "file", "table", "link", "entityview", "dockerrepo"]):
...
def get_folder_synid_from_path(synid_folder_root: str, path: str):
    ...
    for i in range(len(subfolders)):
        synid_folder_children = get_synapse_folder_children(synid_folder_current, include_types=["folder"])
...
def get_file_synid_from_path(synid_folder_root: str, path: str):
    ...
    synid_folder_children = get_synapse_folder_children(synid_folder_dest, include_types=["file"])
    if file_name not in synid_folder_children:
        return None
    return str(synid_folder_children[file_name])

def get_synapse_entity_data_in_csv(synapse_id: str, version: int = None, sep: str = ",", na_strings: List[str] = ["NA"], header: bool = True, check_names: bool = True):
    if version is None:
