# Description: Update release version variables in files given a folder.  
# Author: Xindi Guo
# Date: 2024-02-05
...
option_list <- list( 
  ...
  make_option(c("-o", "--synid_folder_output"), type = "character", default = NA,
              help="Synapse ID of output folder for updated release files (default: write locally)"),
  make_option(c("-u", "--updated_version"), type = "character",
              help=glue("Updated release version number")),
...
if (verbose) {
  print(glue("{now()}: updating release version to {updated_version} in clinical files.."))
}
...
for (i in 1:length(synid_file_children)) {
  ...
  if (verbose) {
    print(glue("{now()}: updating release version from '{file_local}' ({synid_file_child})..."))
  }
  
  raw <- get_synapse_entity_data_in_csv(synid_file_child, na.strings = c(""))
  mod <- raw
  mod$release_version = updated_version
  write.csv(mod, file = file_local, row.names = F, quote = T, na = "")
  
  if (verbose) {
    print(glue("{now()}: release version is updated for '{file_local}' ({synid_file_child})."))
  }
  
  if (!is.na(synid_folder_output)) {
    synid_file_dest <- save_to_synapse(path = file_local, 
                    parent_id = synid_folder_output,
                    prov_name = "update clinical files", 
                    prov_desc = "update release version variable in clinical files", 
                    prov_used = c(synid_file_child), 
                    prov_exec = "https://github.com/Sage-Bionetworks/genie-bpc-pipeline/blob/develop/scripts/release/update_release_version_by_folder.R")
